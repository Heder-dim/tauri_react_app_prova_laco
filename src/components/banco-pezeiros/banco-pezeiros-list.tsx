...
export interface BancoPezeiro {
  id: number;
  nome: string;
  hc: number;
}

export interface BancoPezeirosListProps {
  pezeiros: BancoPezeiro[];
...
export default function BancoPezeirosList({
  pezeiros,
  onRemove,
  onEditar,
}: BancoPezeirosListProps) {
  ...
  function handleIniciarEdicao(pezeiro: BancoPezeiro) {
    setEditandoId(pezeiro.id);
    setNomeEditado(pezeiro.nome);
    setHcEditadoTexto(pezeiro.hc.toString().replace(".", ","));
  }
    ...
    <div className="rounded-2xl bg-white p-5 shadow-sm">
      ...
      {pezeiros.length === 0 ? (
        <p className="py-6 text-center text-sm text-slate-400">
          Nenhum pezeiro cadastrado no banco ainda.
        </p>
      ) : (
        <ul className="divide-y divide-slate-50">
          {pezeiros.map((pezeiro) => {
            const editando = editandoId === pezeiro.id;
            const erro = erroPorLinha[pezeiro.id];

            return (
              <li key={pezeiro.id} className="py-3 first:pt-0 last:pb-0">
                    ...
                    {editando ? (
                      <>
                        <button
                          type="button"
                          aria-label="Salvar"
                          onClick={() => handleSalvarEdicao(pezeiro.id)}
                      ...
                      <>
                        <button
                          ...
                          onClick={() => handleIniciarEdicao(pezeiro)}
                        ...
                        <button
                          type="button"
                          aria-label={`Excluir ${pezeiro.nome}`}
                          onClick={() => handleRemover(pezeiro.id)}
